...
- **`dispatch/`** — One subdirectory per verb (`add/`, `apply/`, `backup/`, `config/`, `console/`, `delete/`, `get/`, `migrate/`, `power/`, `restore/`), each containing one file per noun (`dispatch/get/sessions.rs` implements `manta get sessions`, etc.). Each file exports `pub async fn exec(...)`. Top-level singletons (`dispatch/gen_autocomplete.rs`, `dispatch/gen_man.rs`, `dispatch/upgrade.rs`) live directly under `dispatch/`. `apply/sat_file/` is itself a sub-tree (`render`, `plan`, `dispatch`, `exec`).
...
3. Add the dispatch arm in `crates/manta-cli/src/handlers/<verb>.rs` (which does auth-token bootstrap, argument extraction, and the call to `dispatch::<verb>::<noun>::exec`). For a brand-new verb, also wire `crates/manta-cli/src/handlers/process.rs` to call into the new handler.
