...
- **`handlers/`** — One file per verb (`add`, `apply`, `backup`, `config`, `console`, `delete`, `gen_autocomplete`, `gen_man`, `get`, `log`, `migrate`, `power`, `restore`, `run`, `upgrade`). Each handler does argument extraction, auth-token bootstrap (`get_api_token`), and dispatches to a `commands/<verb>/<noun>.rs::exec(...)`.
- **`commands/`** — One subdirectory per verb (`add/`, `apply/`, `backup/`, `config/`, `console/`, `delete/`, `get/`, `migrate/`, `power/`, `restore/`), each containing one file per noun (`commands/get/sessions.rs` implements `manta get sessions`, etc.). Each file exports `pub async fn exec(...)`. Top-level singletons (`commands/gen_autocomplete.rs`, `commands/gen_man.rs`, `commands/upgrade.rs`) live directly under `commands/`. `apply/sat_file/` is itself a sub-tree (`render`, `plan`, `dispatch`, `exec`).
