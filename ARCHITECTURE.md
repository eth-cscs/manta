...
After Phase 7, the CLI never constructs `StaticBackendDispatcher` and never calls a backend trait method at runtime. Every CLI command (including auth, group-listing, and the previously-direct `apply_session` / `add hardware` / `migrate nodes` / `config_*` paths) goes through `MantaClient`. The CLI's `AppContext::infra` is a one-field `CliInfra { site_name }` — kept minimal because the server holds all real infra (TLS, backend dispatcher, Vault, k8s).
...
> **Residual coupling not addressed by Phase 7:** `csm-rs`, `ochami-rs`, and `manta-backend-dispatcher` remain direct workspace deps of `manta-cli`. The CLI no longer *calls* their APIs, but it still imports types from them — `csm_rs::node::types::NodeDetails` in `http_client.rs` + `output/node.rs`, and `manta_backend_dispatcher::error::Error` across `cli/common/` helpers. Eliminating these imports requires duplicating the wire-type definitions in `manta-shared` and migrating the helper error type — a separate epic ("decouple wire types from csm-rs/ochami-rs"). It is *type* coupling, not *control* coupling; the security/architectural posture is unchanged by leaving it.
