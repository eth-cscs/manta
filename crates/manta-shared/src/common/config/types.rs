...
#[derive(Serialize, Deserialize, Debug)]
/// Legacy unified manta configuration, persisted as TOML under
/// `~/.config/manta/config.toml`. Loaded by both binaries today; will be
/// replaced by `CliConfiguration` (cli.toml) and `ServerConfiguration`
/// (server.toml) once each binary is wired to its own loader.
