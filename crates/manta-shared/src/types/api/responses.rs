...
///
/// Emitted by `POST /v2/redfish-endpoints`,
/// `POST /v2/boot-parameters`, and `POST /v2/groups`.
///
/// # Wire shape
///
/// ```json
...
/// Response for `POST /v2/nodes` — echoes the registered xname.
///
...
/// Response for `POST /v2/sessions` — names of the created CFS
...
/// Response for `POST /v2/ephemeral-env` — the freshly provisioned
/// ephemeral host.
#[derive(Debug, Serialize, Deserialize, ToSchema)]
...
/// `POST /v2/migrate/backup` and `POST /v2/migrate/restore`.
///
/// # Wire shape
///
/// ```json
...
/// Response for `POST /v2/migrate/nodes` — moved xnames plus a
