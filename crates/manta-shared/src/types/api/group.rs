...
//! the HSM group endpoints (`/v2/groups`,
//! `/v2/groups/{name}/members`).

use serde::{Deserialize, Serialize};
use utoipa::ToSchema;

/// Request body for `POST /v2/groups/{name}/members`.
///
...
/// Response body for `POST /v2/groups/{name}/members`.
///
...
/// Request body for `DELETE /v2/groups/{name}/members`.
#[derive(Debug, Serialize, Deserialize, ToSchema)]
...
pub struct GetGroupParams {
  ...
  /// `hsm_group`; used to scope results when `group_name` is
