...
//!
//! - [`node`]  — `POST /v2/nodes`
//! - [`nodes`] — `POST /v2/groups/{label}/members` (assign xnames)
//! - [`group`] — `POST /v2/groups` (+ optional members on creation)
//! - [`hardware`] — `POST /v2/hardware-clusters/{target}/members`
//! - [`boot_parameters`] — `POST /v2/boot-parameters`
//! - [`kernel_parameters`] — `POST /v2/kernel-parameters`
//! - [`redfish_endpoint`] — `POST /v2/redfish-endpoints`
//!
...
pub async fn handle_add(
  ...
  match cli_add.subcommand() {
    ...
    Some(("hardware", m)) => {
      ...
      // POST /v2/hardware-clusters/{target}/members on the server.
      let target = m
