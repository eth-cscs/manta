...
//! `PUT /v2/redfish-endpoints`. The endpoint has no server-side
...
#[cfg(test)]
mod tests {
  /// `--dry-run` parses on `manta apply redfish-endpoints`.
  #[test]
  fn accepts_dry_run() {
    ...
    assert!(
      result.is_ok(),
      "expected --dry-run to parse on `apply redfish-endpoints`: {result:?}"
    );
  }

  /// `-d` short alias also parses (freed by the prior `add
  /// redfish-endpoints` leaf, which swapped `-d/--domain` -> `-D`
  /// via the shared `add_redfish_endpoint_args` helper).
  #[test]
  ...
  /// `-D` short alias for `--domain` still parses on the apply side
  /// (the swap was done in the prior leaf via the shared helper;
  /// this test guards apply-side regressions independently).
  #[test]
  fn accepts_domain_short_alias() {
    ...
    assert!(
      result.is_ok(),
      "expected -D short alias for --domain to parse on apply: {result:?}"
    );
  }
}
