...
// ---------------------------------------------------------------------------
// POST /api/v1/auth/token  +  POST /api/v1/auth/validate
// ---------------------------------------------------------------------------
//
// These endpoints are deliberately publicly reachable (no Bearer extractor):
// they exist so the CLI can exchange Keycloak credentials for a CSM bearer
// token. Mitigations live in `crate::server::auth_middleware` (rate limit,
// body-strip-from-logs) and in the handlers themselves (generic 401 for
// every failure, audit emission via `manta_shared::common::audit`).
