...
pub async fn console_node_ws(
  ...
  Ok(ws.on_upgrade(move |socket| async move {
    ...
    {
      Ok((stdin, stdout)) => {
        run_console_bridge(socket, stdin, stdout, timeout).await;
...
mod tests {
  ...
  #[tokio::test(start_paused = true)]
  async fn resize_text_is_dropped_and_resets_deadline() {
    // Two guarantees:
    //   - deadline resets on any client text frame (including resize)
    //   - the resize JSON is silently consumed, never written to stdin
    let (socket, in_tx, _out_rx) = new_mock_socket();
    ...
    assert!(
      ...
      "resize text frame was forwarded to console stdin (should be silently dropped)"
    );
