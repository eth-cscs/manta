//! Public-router auth handlers (`POST /v2/auth/{token,validate}`).
//!
...
/// POST /v2/auth/token — exchange username/password for a CSM token.
...
/// POST /v2/auth/validate — check whether a CSM token is still valid.
