...
use crate::cli::commands::{delete_group, delete_hw_component_cluster, delete_kernel_parameters, delete_and_cancel_session, delete_configurations_and_derivatives, delete_images};
...
pub async fn handle_delete(
    ...
    if let Some(cli_delete_group) = cli_delete.subcommand_matches("group") {
    ...
    } else if let Some(cli_delete_hw_configuration) = cli_delete.subcommand_matches("hardware") {
        let shasta_token = get_api_token(backend, site_name).await?;
        ...
        .await?;
    } else if let Some(cli_delete_boot_parameters) = cli_delete.subcommand_matches("boot-parameters") {
        let shasta_token = get_api_token(backend, site_name).await?;
    ...
    } else if let Some(cli_delete_redfish_endpoint) = cli_delete.subcommand_matches("redfish-endpoint")
    {
        let shasta_token = get_api_token(backend, site_name).await?;
        ...
        cli_delete.subcommand_matches("kernel-parameters")
    {
    ...
    } else if let Some(cli_delete_configurations) = cli_delete.subcommand_matches("configurations") {
        let shasta_token = get_api_token(backend, site_name).await?;
        let since_opt = if let Some(since) = cli_delete_configurations.get_one::<String>("since") {
        ...
        let until_opt = if let Some(until) = cli_delete_configurations.get_one::<String>("until") {
            let date_time = chrono::NaiveDateTime::parse_from_str(
