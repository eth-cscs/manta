...
pub async fn handle_apply(
    ...
    if let Some(cli_apply_hw) = cli_apply.subcommand_matches("hardware") {
        ...
        let cli_values_file_content_opt: Option<String> = cli_apply_sat_file
            ...
            .and_then(|values_file_path: &std::path::PathBuf| {
                std::fs::read_to_string(values_file_path)
                    .ok()
                    .map(|cli_value_file: String| cli_value_file.replace("__DATE__", &timestamp))
            });
        ...
        let ansible_passthrough_env: Option<String> = settings.get("ansible-passthrough").ok();
        ...
        let overwrite: bool = cli_apply_sat_file.get_flag("overwrite-configuration");
        ...
        cli_apply.subcommand_matches("kernel-parameters")
    {
        ...
        if let Some(cli_apply_boot_nodes) = cli_apply_boot.subcommand_matches("nodes") {
            ...
            let new_boot_image_id_opt: Option<&String> = cli_apply_boot_nodes.get_one("boot-image");
