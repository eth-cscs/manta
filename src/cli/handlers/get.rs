...
use manta_backend_dispatcher::interfaces::hsm::{group::GroupTrait, redfish_endpoint::RedfishEndpointTrait};
use manta_backend_dispatcher::types::bss::BootParameters;
use crate::common::{authorization::{get_groups_names_available, validate_target_hsm_members}, authentication::get_api_token};
use crate::cli::commands::{get_group, get_hardware_cluster, get_hardware_node, get_configuration, get_session, get_template, get_cluster, get_nodes, get_images, get_boot_parameters, get_kernel_parameters};
...
pub async fn handle_get(
    ...
    if let Some(cli_get_groups) = cli_get.subcommand_matches("groups") {
        ...
        if let Some(cli_get_hardware_cluster) = cli_get_hardware.subcommand_matches("cluster") {
            let shasta_token = get_api_token(backend, site_name).await?;
            ...
            .await;
        } else if let Some(cli_get_hardware_node) = cli_get_hardware.subcommand_matches("node") {
            let shasta_token = get_api_token(backend, site_name).await?;
        ...
        let pattern: Option<&String> = cli_get_configuration.get_one::<String>("pattern");
        ...
        let shasta_token = get_api_token(backend, site_name).await?;
        let hsm_group_name_arg_opt: Option<&String> = cli_get_session.get_one("hsm-group");
        ...
        let shasta_token = get_api_token(backend, site_name).await?;
        let hsm_group_name_arg_opt: Option<&String> = cli_get_cluster.get_one("HSM_GROUP_NAME");
        let target_hsm_group_vec = get_groups_names_available(
            backend,
            &shasta_token,
            hsm_group_name_arg_opt,
        ...
        .await?;
    } else if let Some(cli_get_boot_parameters) = cli_get.subcommand_matches("boot-parameters") {
        let shasta_token = get_api_token(backend, site_name).await?;
        let hsm_group_name_arg_opt: Option<&String> = cli_get_boot_parameters.get_one("hsm-group");
    ...
    } else if let Some(cli_get_kernel_parameters) = cli_get.subcommand_matches("kernel-parameters") {
        let shasta_token = get_api_token(backend, site_name).await?;
        let hsm_group_name_arg_opt: Option<&String> = cli_get_kernel_parameters.get_one("hsm-group");
        ...
        .await;
    } else if let Some(cli_get_redfish_endopints) = cli_get.subcommand_matches("redfish-endpoints") {
        let shasta_token = get_api_token(backend, site_name).await?;
