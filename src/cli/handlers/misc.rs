...
use crate::cli::commands::{validate_local_repo, add_nodes_to_hsm_groups, remove_nodes_from_hsm_groups};
...
pub async fn handle_misc(
    ...
    if let Some(cli_validate_local_repo) = cli_root.subcommand_matches("validate-local-repo") {
        ...
        let repo_path = cli_validate_local_repo
            ...
            .unwrap();
        validate_local_repo::exec(shasta_root_cert, gitea_base_url, &gitea_token, repo_path).await;
        ...
        .await?;
    } else if let Some(cli_remove_nodes) = cli_root.subcommand_matches("remove-nodes-from-groups") {
        let shasta_token = get_api_token(backend, site_name).await?;
