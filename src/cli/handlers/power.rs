...
use crate::cli::commands::{power_on_cluster, power_on_nodes, power_off_cluster, power_off_nodes, power_reset_cluster, power_reset_nodes};
...
pub async fn handle_power(
    ...
    if let Some(cli_power_on) = cli_power.subcommand_matches("on") {
        ...
        if let Some(cli_power_reset_cluster) = cli_power_reset.subcommand_matches("cluster") {
            ...
            .await?;
        } else if let Some(cli_power_reset_node) = cli_power_reset.subcommand_matches("nodes") {
            let shasta_token = get_api_token(backend, site_name).await?;
