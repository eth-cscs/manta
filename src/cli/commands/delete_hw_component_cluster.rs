...
pub async fn exec(
  ...
  if !pattern_element_vec.len().is_multiple_of(2) {
    bail!(
      "Error in pattern: odd number of elements after \
       group name. Expected pairs of \
       <hw component>:<count>. \
  ...
  // ***********************************************************
  // CALCULATE FINAL HSM GROUP HW COMPONENT COUNTERS (SUMMARY)
  // AND DELTAS
  ...
  {
    let new_counter: usize = *target_hsm_hw_component_summary
      .get(hw_component)
      .ok_or_else(|| {
        Error::msg(format!(
  ...
  if !common::user_interaction::confirm(&confirm_message, false) {
