...
pub fn subcommand_apply_image(hsm_group: Option<&String>) -> Command {
    let mut apply_image = Command::new("image")
        .aliases(["i", "img", "imag"])
        .arg_required_else_help(true)
        ...
        .arg(arg!(-w --"watch-logs" "Watch logs. Hooks stdout to see container running ansible scripts"))
        .group(ArgGroup::new("file_or_repos_and_config-name").args(["file", "repo-path"]).required(true));

    match hsm_group {
        Some(_) => {}
        None => {
            apply_image =
                apply_image.arg(arg!(-H --"hsm-group" <VALUE> "hsm group name").required(true))
        }
    };
