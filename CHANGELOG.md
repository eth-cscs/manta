...
- `cli.toml`'s `manta_server_url` field is now **required**. The CLI's authentication flow (env-var, cached-file, and interactive Keycloak login paths in `authentication.rs`) is forwarded to the named manta server via the new auth endpoints. Users must point `manta_server_url` at a reachable `manta-server` instance.
  - A long tail of CLI handlers (`apply_session`, `handlers/add hardware`, `handlers/migrate nodes`, `config_show/set/unset hsm/parent-hsm`) still talks to the backend directly via `StaticBackendDispatcher`. Migrating those to the server is tracked as a follow-up; `csm-rs` and `ochami-rs` remain direct deps of `manta-cli` for that reason and for the wire-type definitions.
...
- New server endpoints `POST /api/v1/auth/token` and `POST /api/v1/auth/validate`. The CLI uses them to exchange Keycloak credentials for a CSM/OCHAMI token without ever calling the backend directly. Both endpoints are on a separate sub-router (no `Authorization: Bearer` middleware) with four defensive layers: per-source-IP rate limit (`[server].auth_rate_limit_per_minute`, default 60 in samples; in-process token-bucket — operators should also rate-limit at the reverse proxy), generic `401 invalid credentials` on every failure (no username enumeration; detail stays in server `tracing::warn!`), structured per-attempt audit event to the configured Kafka producer (never the password), and explicit body redaction on `/auth/*` log spans. `[server]` gains `auth_rate_limit_per_minute`; `ServerState` gains an `auditor` field wired from `[auditor.kafka]` in `server.toml`. Forwarding the resolved client IP to Keycloak as `X-Forwarded-For` is deferred — it requires extending `AuthenticationTrait::get_api_token` in `manta-backend-dispatcher` (sibling repo).
- New server endpoints `GET /api/v1/groups/available` (HSM groups the bearer token can access) and `GET /api/v1/groups/all` (every group in the system). Match the `MantaClient::get_available_groups` / `MantaClient::get_all_groups` methods used by the CLI's authorization-helper migration.
- Server-side authorization closed: `POST /api/v1/sessions`, `POST /api/v1/hardware-clusters/{target}/members`, `DELETE /api/v1/hardware-clusters/{target}/members`, `POST /api/v1/hardware-clusters/{target}/configuration`, and `POST /api/v1/migrate/nodes` now call `service::group::validate_hsm_group_access` (and, where applicable, the existing `validate_target_hsm_members`) before dispatching to the service layer. Previously these handlers trusted the backend to reject; the new checks make access errors fail fast with usable messages.
- Deleted the dead `POST /api/v1/sessions/apply` endpoint and `ApplySessionRequest` struct. The CLI's `MantaClient::create_session` always used `POST /api/v1/sessions`; the apply variant had no callers.

### Refactor (Phase 7 — CLI ↔ backend decoupling)

- `crates/manta-cli` no longer constructs `StaticBackendDispatcher` and no longer calls any backend trait method at runtime. Every CLI command goes through `MantaClient`. The eight remaining direct-backend touchpoints across `apply_session`, `handlers/add hardware`, `handlers/migrate nodes`, `config_show`, `config_set_hsm`, `config_set_parent_hsm`, `config_set_hsm_common`, and `config_unset_parent_hsm` have all moved to the corresponding `MantaClient` calls. `config_unset_parent_hsm` lost a dead backend call entirely (it was fetching the accessible-groups list and never using it).
...
- Split `crates/manta-cli/src/cli/http_client.rs` (1254 LOC, 50 methods in one `impl` block) into a directory with 17 per-resource sub-modules mirroring the server's `handlers/` layout. The `MantaClient` struct, constructor, helpers, `QueryBuilder`, and `ws_base_url` stay in `mod.rs`; each sub-module carries a separate `impl MantaClient { ... }` block. External callers (`client.get_sessions(...)`, `client.add_node(...)`, …) are byte-identical — the split is purely organisational.
- Moved three large inline `#[cfg(test)] mod tests { ... }` blocks to sibling `tests.rs` files. Production-file LOC shrinkage: `service/hw_cluster/mod.rs` 1255 → 71 (−94%), `server/common/node_ops.rs` 681 → 343 (−50%), `shared/sat_file.rs` 844 → 647 (−23%). For the flat-file modules (`node_ops.rs`, `sat_file.rs`) Rust 2018+ resolves `mod tests;` to a sibling directory of the same name.
- Converted `manta-server` from binary-only to `[lib] + [[bin]]`. New `crates/manta-server/src/lib.rs` declares the six top-level modules as `pub mod`; `main.rs` becomes a thin bootstrap that calls into the library. Moved `crates/manta-server/src/server/{tests.rs, integration_tests.rs}` (1864 LOC combined) to `crates/manta-server/tests/{server_routes.rs, integration.rs}` per Rust convention — separate compilation unit, no longer recompiles when production code changes. Promoted `server::handlers::to_handler_error` from `pub(crate)` to `pub` for the integration tests to reach. Test suite still 313, now split across separate test binaries.
- Collapsed the two compatibility shims `crates/manta-cli/src/common.rs` and `crates/manta-server/src/common/mod.rs` that re-exported under a `crate::common::*` namespace. Every import now states its origin: either `manta_shared::common::*` (workspace-shared) or `crate::{cli,server}::common::*` (crate-local). The server-local `InfraContext` moved with the shim from `crate::common::app_context` to `crate::server::common::app_context`. ~90 import sites rewritten across the workspace.
...
- Try to fix github workflow to build artifacts
- Release manta-cli version 1.62.8
...
- Try to fix github workflow to build artifacts
- Release manta-cli version 1.62.7
...
- Try to fix github workflow to build artifacts
- Release manta-cli version 1.62.6
...
- Try to fix github workflow to build artifacts
- Release manta-cli version 1.62.5
...
- Try to fix github workflow to build artifacts
- Release manta-cli version 1.62.4
...
- Update cargo dist
- Release manta-cli version 1.62.1
...
- Release manta-cli version 1.59.9-beta.13
...
- Release manta-cli version 1.59.9-beta.12
...
- Release manta-cli version 1.59.9-beta.11
...
- Release manta-cli version 1.59.9-beta.10
...
- Release manta-cli version 1.59.9-beta.9
...
- Release manta-cli version 1.59.9-beta.8
...
- Release manta-cli version 1.59.9-beta.7
...
- Release manta-cli version 1.59.9-beta.4
...
- Release manta-cli version 1.59.9-beta.3
...
- Refactor code
- Refactor code
- Release manta-cli version 1.59.9-beta.2
...
- Release manta-cli version 1.59.5
...
- Update Cargo.toml
- Clean Cargo.lock
- Release manta-cli version 1.59.3
...
- Release manta-cli version 1.59.1
...
- Release manta-cli version 1.59.0
...
- Update Cargo.toml
- Update Cargo.lock
- Release manta-cli version 1.58.9
...
- Update Cargo.lock
- Release manta-cli version 1.58.8
...
- Release manta-cli version 1.57.2
...
- Update Cargo.lock
- Release manta-cli version 1.56.17
