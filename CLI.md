...
> **No server-side pre-flight.** Validation is client-side only: dangling `image_ref` references and image cycles fail before anything is sent. There is no whole-file check against live CFS / IMS / `cray-product-catalog` state — `POST /sat-file/validate` was removed in `f9160d26`. Anything that depends on live backend state surfaces on the first per-element call, which means a SAT file can fail partway through with earlier elements already applied.
