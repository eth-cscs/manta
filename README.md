...
| [SECURITY.md](SECURITY.md) | how to report vulnerabilities, supported versions, security-model summary |
...
- Read-only JWT role: callers whose bearer token's `realm_access.roles` claim contains `manta-read-only` are refused with `403 Forbidden` on every `POST`/`PUT`/`PATCH`/`DELETE` under `/api/v1/*`. Safe methods (`GET`/`HEAD`) and `/api/v1/auth/*` (login) pass through. The CLI mirrors this gate locally for fast feedback by decoding the same role from the JWT before issuing the request; the server is the authoritative boundary. Provision the role in Keycloak; no `server.toml` knob.
