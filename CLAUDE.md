...
`manta-shared`'s pure helpers (audit, jwt_ops, kafka, config loader, sat-file parser, network probe) return `manta_shared::common::error::MantaError`; server code maps these to `BackendError` at call sites via `crates/manta-server/src/wire_conv.rs::to_backend`.
